...
def get_image_ouverture_rdv(ouverture_rdv, related_statut):
    if (ouverture_rdv.statut == related_statut):
        return 'checked_checkbox'
    return 'empty_checkbox'

def get_comments_ouverture_rdv(ouverture_rdv):
    if ouverture_rdv.statut == ouvertureRdv.PRESENT_NC:
        return  ouverture_rdv.motif
    return ""
    
def get_newdateheure_ouverture_rdv(ouverture_rdv):
    if ouverture_rdv.statut == ouvertureRdv.REPORT:
        return  ouverture_rdv.dateheure_nouveauRdv
    return None

def get_newdate_ouverture_rdv(ouverture_rdv):
    newdateheure = get_newdateheure_ouverture_rdv(ouverture_rdv)
    if newdateheure:
        return  newdateheure.date().strftime("%d/%m/%Y")
    return ''

def get_newtime_ouverture_rdv(ouverture_rdv):
    newdateheure = get_newdateheure_ouverture_rdv(ouverture_rdv)
    if newdateheure:
...
def evenement_est(response, lettre_commande):
    ...
    elements.append(Paragraph(u"<font size='12'><b>Nom du prestataire: </b>%s</font>" % (lettre_commande.site.prestataire.nom,), location_style))
    elements.append(Paragraph(u"<font size='12'><b>Lieu de réalisation: </b>%s</font>" % (lettre_commande.site.nom,), location_style))
    ...
    tableData = [[u"Nom-Prénom du DE:", lettre_commande.nom+" "+lettre_commande.prenom, "", "",u"N° Identifiant:" ,lettre_commande.polemploi_id],
                 ["Date du premier RDV:", lettre_commande.ouverture_rdv.dateheure.date().strftime("%d/%m/%Y"), "", "","Heure du RDV:" ,lettre_commande.ouverture_rdv.dateheure.time().strftime("%H:%M")],
                 ]
    ...
    demarragePresta_data = [[eval(get_image_ouverture_rdv(lettre_commande.ouverture_rdv, ouvertureRdv.PRESENT_C)), "A DEMARRE LA PRESTATION"],]
